...
## Паралельний імпорт файлів усередині однієї клініки

### Мотивація

Послідовний імпорт великої клініки (десятки тисяч файлів) міг тривати годинами. Оскільки
кожна клініка вже захищена своїм `fcntl`-локом (одна клініка = один процес), паралелізм
має сенс **усередині** обробки файлів однієї клініки — по кожному файлу окремо.

### Реалізація

`import_clinic.py` використовує `multiprocessing.Pool` — кожен воркер це окремий OS-процес
(не тред, через GIL для CPU-bound SHA256-хешування треди не дали б приросту). Кількість
воркерів визначається автоматично:
...
Override вручну через змінну середовища:

```bash
IMPORT_WORKERS=1 docker exec mayan-app-1 nice -n 19 \
  /opt/mayan-edms/bin/python /var/lib/mayan/import_clinic.py 0602   # послідовно
IMPORT_WORKERS=2 docker exec mayan-app-1 nice -n 19 \
  /opt/mayan-edms/bin/python /var/lib/mayan/import_clinic.py 0602   # 2 потоки
```

### Критична деталь: закриття DB-з'єднання ПЕРЕД fork()

`multiprocessing.Pool` створює дочірні процеси через `fork()`. Якщо на момент форку
в батьківському процесі відкрите Django DB-з'єднання (а воно відкривається ще на етапі
`Role.objects.get(...)` на старті скрипта) — кожен дочірній процес успадковує копію
того самого файлового дескриптора TCP-сокета до PostgreSQL. Кілька процесів, що
одночасно чіпають той самий сокет ще до відкриття власних нових з'єднань, ламають
протокол з точки зору сервера:

```text
psycopg.OperationalError: consuming input failed: server closed the connection unexpectedly
```
Це стається **навіть якщо** кожен воркер викликає `db.connections.close_all()` на своєму
старті — того вже недостатньо, бо пошкодження відбувається до цього виклику. Фікс —
закривати з'єднання явно **в батьківському процесі, перед створенням `Pool`**:

```python
from django import db
db.connections.close_all()   # ОБОВ'ЯЗКОВО перед mp.Pool(workers)
with mp.Pool(workers) as pool:
    results = pool.map(worker_import_file, args)
db.connections.close_all()   # і після — з'єднання батька теж могло стати невалідним
```

### Агрегація статистики

Глобальний `counter` не працює для підрахунку в паралельному режимі — кожен форкнутий
процес має власну ізольовану копію пам'яті, зміни в дочірньому процесі ніколи не
повертаються в батьківський. Тому `import_file()` повертає статус рядком
(`'ok'/'skip'/'linked'/'error'`), а підсумок рахується в батьківському процесі з
результатів `pool.map()`.

### Кілька клінік одночасно

Оскільки кожна клініка має власний `fcntl`-лок, паралельний запуск різних клінік
безпечний без жодних додаткових змін — просто через `&`/`wait`:

```bash
/opt/mayan/sync_and_import.sh --no-rsync 0501 &
/opt/mayan/sync_and_import.sh --no-rsync 0502 &
/opt/mayan/sync_and_import.sh --no-rsync 0503 &
wait
```

⚠️ Комбінувати одночасний запуск кількох клінік **і** `IMPORT_WORKERS > 1` в кожній
варто обережно — сумарна кількість паралельних DB-з'єднань і CephFS-операцій зростає
мультиплікативно. Починай з малого (`IMPORT_WORKERS=2`, 2-3 клініки одночасно) і
збільшуй, спостерігаючи за навантаженням PostgreSQL (`pg_stat_activity`) і диском.
